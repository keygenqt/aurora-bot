...
impl FlutterProjectReportIncoming {
    ...
    pub fn new() -> Box<FlutterProjectReportIncoming> {
        print_debug!("> {}: new()", Self::name());
        Box::new(Self { id: None })
    }

    pub fn new_id(id: String) -> Box<FlutterProjectReportIncoming> {
    ...
    fn run(&self, send_type: OutgoingType) -> Box<dyn TraitOutgoing> {
        ...
        match models.iter().count() {
            1 => match Self::run(models.first().unwrap().clone(), &send_type) {
                Ok(result) => result,
                Err(_) => StateMessageOutgoing::new_error(tr!("не удалось создать отчет")),
            },
