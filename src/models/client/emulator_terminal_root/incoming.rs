...
pub struct EmulatorTerminalRootIncoming {
    id: Option<String>,
}

impl EmulatorTerminalRootIncoming {
    pub fn name() -> String {
        serde_variant::to_variant_name(&ClientMethodsKey::EmulatorTerminalRoot)
            .unwrap()
            .to_string()
    }

    pub fn new() -> Box<EmulatorTerminalRootIncoming> {
        Box::new(Self { id: None })
    }

    pub fn new_id(id: String) -> Box<EmulatorTerminalRootIncoming> {
...
impl TraitIncoming for EmulatorTerminalRootIncoming {
    fn run(&self, send_type: OutgoingType) -> Box<dyn TraitOutgoing> {
        // Search
        let key = EmulatorTerminalRootIncoming::name();
        ...
        fn _run(emulator: EmulatorModel) -> Box<dyn TraitOutgoing> {
            if !emulator.is_running {
                ...
                // Run command
                let command = format!(
                    "ssh -o 'ConnectTimeout=2' -o 'StrictHostKeyChecking=no' root@localhost -p 2223 -i {}", emulator.key
                );
                // Try run terminal
        ...
        match models.iter().count() {
            ...
            _ => Box::new(EmulatorModelSelect::select(key, models, |id| {
                *EmulatorTerminalRootIncoming::new_id(id)
            })),
        }
    }
}
