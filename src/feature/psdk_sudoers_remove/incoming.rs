...
pub struct PsdkSudoersRemoveIncoming {
    id: Option<String>,
}

impl PsdkSudoersRemoveIncoming {
    pub fn name() -> String {
        serde_variant::to_variant_name(&ClientMethodsKey::PsdkSudoersRemove)
            .unwrap()
            .to_string()
    }

    pub fn new() -> Box<PsdkSudoersRemoveIncoming> {
        print_debug!("> {}: new()", Self::name());
        Box::new(Self { id: None })
    }

    pub fn new_id(id: String) -> Box<PsdkSudoersRemoveIncoming> {
        print_debug!("> {}: new_id(id: {})", Self::name(), id);
        Box::new(Self { id: Some(id) })
    }

    fn select(&self, id: String) -> PsdkSudoersRemoveIncoming {
...
impl TraitIncoming for PsdkSudoersRemoveIncoming {
    fn run(&self, send_type: OutgoingType) -> Box<dyn TraitOutgoing> {
        // Search
        let key = PsdkSudoersRemoveIncoming::name();
